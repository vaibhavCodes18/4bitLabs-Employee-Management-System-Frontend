...
const AnalystDashboard = () => {
  ...
  const handleEditBatch = async (e) => {
    ...
    try {
      ...
      setBatches(batches.map((b) => (b.id === currentBatch.id ? response.data : b)));
  ...
  const stats = [
    { label: "Total Batches", value: totalBatches, icon: FaLayerGroup, color: "bg-blue-500" },
    { label: "Active", value: activeBatches, icon: FaCheckCircle, color: "bg-green-500" },
    { label: "Upcoming", value: upcomingBatches, icon: FaClock, color: "bg-yellow-500" },
    { label: "Completed", value: completedBatches, icon: FaCheckCircle, color: "bg-purple-500" },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans flex">
      {/* Sidebar (unchanged) */}
      <aside className={`${sidebarOpen ? "w-64" : "w-20"} bg-white shadow-lg transition-all duration-300 hidden md:block relative`}>
        <div className="p-4 flex items-center space-x-2 border-b">
          <div className="bg-indigo-600 p-2 rounded-lg">
            <FaMicrochip className="text-white text-xl" />
          </div>
          {sidebarOpen && <span className="text-xl font-bold text-gray-800">4bitlabs</span>}
        </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="hidden sm:block">
                <p className="text-sm font-semibold text-gray-800">{user.name || "Analyst"}</p>
        ...
        <div className="p-6">
          {activeView === "dashboard" && (
            <>
              ...
              <p className="text-gray-500 mb-6">Welcome back, {user.name}! Here's your batch overview.</p>
                      ...
                      {batches.slice(0, 5).map((batch) => (
                        <tr key={batch.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="whitespace-nowrap">
                            <button onClick={() => openViewModal(batch)} className="text-blue-600 hover:text-blue-800 mr-2">
                              <FaEye />
                            </button>
                            <button onClick={() => openEditModal(batch)} className="text-indigo-600 hover:text-indigo-800 mr-2">
                              <FaEdit />
                            </button>
                            <button onClick={() => openDeleteModal(batch)} className="text-red-600 hover:text-red-800">
                              <FaTrash />
                            </button>
                          </td>
                        </tr>
          ...
          {activeView === "reports" && (
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <p className="text-gray-600">Coming soon: Visual reports on batch performance, student progress, etc.</p>
            </div>
          )}
        </div>
      </main>
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <DetailItem label="Students Count" value={currentBatch.studentsCount} />
            </div>
            <div className="flex justify-end mt-6">
              <button onClick={closeModals} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                Close
              </button>
            </div>
          </div>
      ...
      {showDeleteModal && currentBatch && (
        <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">Are you sure you want to delete batch "{currentBatch.name}"?</p>
            <div className="flex justify-end space-x-3">
              <button onClick={closeModals} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={handleDeleteBatch} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">
                Yes, Delete
              </button>
            </div>
          </div>
